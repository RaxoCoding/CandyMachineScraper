...
async function getMetaData(apiKeyId, apiSecretKey, candyId, _callback) {
    ...
    await axios(config)
        .then(async function(response) {
            if (response.data.is_candy_machine) {
                config = {
                    method: 'post',
                    ...
                    data: {
                        ...
                        candy_machine_contract_version: response.data.candy_machine_contract_version
                    }
                };

                await axios(config)
                    .then(function(response) {
                        _callback({ state: 'success', data: response.data });
                        return;
                    })
